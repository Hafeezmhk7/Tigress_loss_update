...
def batch_eval(gru, test_data, cutoff=[20], batch_size=512, mode='conservative', item_key='ItemId', session_key='SessionId', time_key='Time'):
    ...
    for in_idxs, out_idxs in data_iterator(enable_neg_samples=False, reset_hook=reset_hook):
        ...
        oscores = O.T
        tscores = oscores[out_idxs.long(), torch.arange(out_idxs.shape[0], device=gru.device)].unsqueeze(0)
        if mode == 'standard': ranks = (oscores > tscores).sum(dim=0) + 1
        elif mode == 'conservative': ranks = (oscores >= tscores).sum(dim=0)
        elif mode == 'median':  ranks = (oscores > tscores).sum(dim=0) + 0.5*((oscores == tscores).sum(dim=0) + 1)
        else: raise NotImplementedError
        for c in cutoff:
            mask = ranks <= c
            recall[c] += mask.sum().cpu().numpy()
            mrr[c] += (mask.float() / ranks.float()).sum().cpu().numpy()
            ndcg[c] += (mask.float() / torch.log2(ranks.float() + 1)).sum().cpu().numpy()
